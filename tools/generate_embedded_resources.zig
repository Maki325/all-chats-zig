...
fn embedFile(input_abs_path: []const u8, top_dir: std.fs.Dir, symlink_dir: std.fs.Dir, deps_writer: anytype, output_writer: anytype) !void {
    const resource_name = std.fs.path.basename(input_abs_path);
    const output_dir_path = try symlink_dir.realpathAlloc(alloc, ".");
    defer alloc.free(output_dir_path);

    const top_dir_path = try top_dir.realpathAlloc(alloc, ".");
    defer alloc.free(top_dir_path);

    const output_abs_path = try std.fmt.allocPrint(alloc, "{s}/{s}", .{ output_dir_path, resource_name });
    defer alloc.free(output_abs_path);

    const relative_path = try std.fs.path.relative(alloc, top_dir_path, output_abs_path);
...
fn walkFiles(top_dir: std.fs.Dir, output_writer: anytype, deps_writer: anytype, input_dir: std.fs.Dir, output_dir: std.fs.Dir, rel_path: []const u8) !void {
    ...
    while (try it.next()) |elem| {
        ...
        switch (elem.kind) {
            .directory => {
                ...
                try walkFiles(top_dir, output_writer, deps_writer, new_input_dir, new_output_dir, new_rel_path);
            },
            .file => {
                const input_abs_path = try input_dir.realpathAlloc(alloc, elem.name);
                defer alloc.free(input_abs_path);

                try embedFile(input_abs_path, top_dir, output_dir, deps_writer, output_writer);
            },
            else => {
                continue;
            },
...
pub fn main() !void {
    ...
    for (3..args.len) |input_idx| {
        ...
        if (input_stat.kind == .directory) {
            ...
            try walkFiles(output_dir, output_writer, deps_writer, input_dir, output_dir, ".");
        } else {
            const input_abs_path = try cwd.realpathAlloc(alloc, input);
            defer alloc.free(input_abs_path);

            try embedFile(input_abs_path, output_dir, output_dir, deps_writer, output_writer);
        }
    }
